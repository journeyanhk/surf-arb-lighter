...
logger = logging.getLogger("arbitrage.adapters.rblighter")

RBLIGHTER_TERMINAL_STATUSES = {"filled"}


class RBLighterAdapter:
    name = "rblighter"

    def __init__(self, settings: Settings, client: httpx.AsyncClient | None = None) -> None:
        self.settings = settings
        self.http = client or httpx.AsyncClient(base_url=settings.rblighter_base_url, timeout=10)
        self._signer = None
    ...
    async def _account(self, *, allow_cache: bool = True) -> dict:
        if self.settings.rblighter_account_index is None:
            raise RuntimeError("RBLIGHTER_ACCOUNT_INDEX is required")

        async def fetch() -> dict:
            response = await self.http.get("/api/v1/account", params={"by": "index", "value": self.settings.rblighter_account_index})
            response.raise_for_status()
            data = response.json()
            accounts = data.get("accounts") or ([data["account"]] if "account" in data else [])
            if not accounts:
                raise RuntimeError("RBLighter account response contains no account")
            return accounts[0]
    ...
    def _equity_from(account: dict) -> Decimal:
        ...
        if value is None:
            raise RuntimeError("RBLighter account response has no documented total account value field")
    ...
    def _position_from(account: dict, mapping: MarketMapping) -> Position:
        ...
        row = next(
            (
                ...
                == mapping.rblighter_market_index
            ),
            None,
        )
        if not row:
    ...
    async def _mark_price(self, mapping: MarketMapping) -> Decimal:
        market = mapping.rblighter_market_index
        ...
        try:
            ...
            raise RuntimeError(f"RBLighter market_stats mark price unavailable for market {market}") from exc
        mark = self._mark_prices.get(market)
        if mark is None or mark <= 0:
            raise RuntimeError(f"RBLighter market_stats returned no positive mark price for market {market}")
        return mark
    ...
    async def _mark_stream_loop(self, mapping: MarketMapping, ready: asyncio.Event) -> None:
        ...
        while True:
            try:
                async with websockets.connect(self.settings.rblighter_ws_url, ping_interval=20, ping_timeout=20) as ws:
                    ...
                    async for raw in ws:
                        ...
                        if first:
                            logger.info(
                                "RBLighter 标记价流已连接 market_index=%s symbol=%s mark_price=%s",
                                market,
                                mapping.rblighter_symbol,
                                mark,
                            )
                ...
                logger.warning("RBLighter 标记价流断开，%s 秒后重连：%s", delay, exc)
    ...
    async def _ensure_position_mark(self, position: Position, mapping: MarketMapping) -> Position:
        ...
        return replace(
            position,
            mark_price=mark,
            updated_at_ms=self._mark_updated_at_ms.get(mapping.rblighter_market_index),
        )
    ...
    async def position(self, mapping: MarketMapping) -> Position:
        if not self.settings.dry_run:
            ...
            cached = self._account_positions.get(mapping.rblighter_market_index)
    ...
    async def account_snapshot(self, mapping: MarketMapping) -> AccountSnapshot:
        ...
        if available is None:
            raise RuntimeError("RBLighter account response has no available_balance field")
    ...
    async def open_orders(self, mapping: MarketMapping) -> list[dict]:
        ...
        auth, error = signer.create_auth_token_with_expiry(
            api_key_index=self.settings.rblighter_api_key_index
        )
        if error or not auth:
            raise RuntimeError(f"RBLighter auth token generation failed: {error or 'empty token'}")
        response = await self.http.get(
            "/api/v1/accountActiveOrders",
            params={"account_index": self.settings.rblighter_account_index, "market_id": mapping.rblighter_market_index},
            headers={"Authorization": auth},
        )
        response.raise_for_status()
        rows = response.json().get("orders", [])
        merged = {str(row.get("order_id") or row.get("order_index")): row for row in rows}
        merged.update(self._active_orders.get(mapping.rblighter_market_index, {}))
        return list(merged.values())

    async def _auth_token(self) -> str:
        signer = await self._configured_signer()
        auth, error = signer.create_auth_token_with_expiry(api_key_index=self.settings.rblighter_api_key_index)
        if error or not auth:
            raise RuntimeError(f"RBLighter auth token generation failed: {error or 'empty token'}")
        return auth
    ...
    async def _account_stream_loop(self, mapping: MarketMapping, ready: asyncio.Event) -> None:
        ...
        while True:
            try:
                ...
                channel = f"account_market/{mapping.rblighter_market_index}/{self.settings.rblighter_account_index}"
                async with websockets.connect(self.settings.rblighter_ws_url, ping_interval=20, ping_timeout=20) as ws:
                    ...
                    async for raw in ws:
                        ...
                        if payload.get("type") in {"update/account_market", "subscribed/account_market"}:
                            if not ready.is_set():
                                logger.info("RBLighter 账户订单/成交流已订阅 market_index=%s", mapping.rblighter_market_index)
                            ready.set()
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                ready.clear()
                self._account_positions.pop(mapping.rblighter_market_index, None)
                logger.warning("RBLighter 账户流断开，%s 秒后重连：%s", delay, exc)
                await asyncio.sleep(delay)
                delay = min(delay * 2, 30)
    ...
    def _apply_account_message(self, payload: dict, mapping: MarketMapping) -> None:
        ...
        if "position" in payload:
            ...
            for item in rows:
                ...
                # position. Some RBLighter snapshots omit the market id.
                if raw_market is None and len(rows) == 1:
                    row = item
                    break
                try:
                    if int(raw_market) == mapping.rblighter_market_index:
                        row = item
                        break
            ...
            if row is None:
                logger.warning(
                    "忽略无法匹配市场的 RBLighter 仓位推送 market_index=%s rows=%s",
                    mapping.rblighter_market_index,
                    len(rows),
                )
            else:
                normalized = dict(row)
                normalized.setdefault("market_id", mapping.rblighter_market_index)
                ...
                self._account_positions[mapping.rblighter_market_index] = replace(
                    position,
                    updated_at_ms=updated_at_ms,
                )
        for order in self._rows(payload.get("orders")):
            if int(order.get("market_index", order.get("market_id", -1))) != mapping.rblighter_market_index:
                continue
            key = order.get("client_order_id") or order.get("client_order_index")
            if key is None:
                continue
            status = str(order.get("status", "")).lower()
            terminal = status in RBLIGHTER_TERMINAL_STATUSES or status.startswith("canceled")
            active_key = str(order.get("order_id") or order.get("order_index"))
            active = self._active_orders.setdefault(mapping.rblighter_market_index, {})
            if terminal:
        ...
        for trade in self._rows(payload.get("trades")):
            if int(trade.get("market_id", -1)) != mapping.rblighter_market_index:
                continue
            account = str(self.settings.rblighter_account_index)
    ...
    async def _configured_signer(self):
        if self._signer is None:
            if self.settings.rblighter_account_index is None:
                raise RuntimeError("RBLIGHTER_ACCOUNT_INDEX is required")
            if self.settings.rblighter_api_key_index is None or not self.settings.rblighter_api_private_key:
                raise RuntimeError("RBLighter API key is required for authenticated account reads")
            try:
                import lighter
            except ImportError as exc:
                raise RuntimeError("install the 'live' extra to access authenticated RBLighter data") from exc
            self._signer = lighter.SignerClient(
                url=self.settings.rblighter_base_url,
                api_private_keys={self.settings.rblighter_api_key_index: self.settings.rblighter_api_private_key},
                account_index=self.settings.rblighter_account_index,
            )
            error = self._signer.check_client()
            if error:
                self._signer = None
                raise RuntimeError(f"RBLighter API key validation failed: {error}")
        return self._signer
    ...
    async def place_order(self, mapping: MarketMapping, request: OrderRequest) -> OrderResult:
        ...
        scale_qty = Decimal(10) ** mapping.rblighter_qty_precision
        scale_price = Decimal(10) ** mapping.rblighter_price_precision
        ...
        tx, response, err = await signer.create_order(
            market_index=mapping.rblighter_market_index,
        ...
        if response is None or int(response.code) != 200:
            message = getattr(response, "message", None) or "empty RBLighter transaction response"
        ...
        try:
            ...
            return replace(current, status="UNKNOWN", error=f"no terminal RBLighter account event before timeout; tx={response.tx_hash}")

    async def cancel_order(self, mapping: MarketMapping, order_id: str) -> bool:
        signer = await self._signer_client()
        _, _, err = await signer.cancel_order(mapping.rblighter_market_index, int(order_id))
        return err is None
